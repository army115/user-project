class Nonpay {
  bool ok;
  List<Nonuser> nonuser;

  Nonpay({this.ok, this.nonuser});

  Nonpay.fromJson(Map<String, dynamic> json) {
    ok = json['ok'];
    if (json['nonuser'] != null) {
      nonuser = new List<Nonuser>();
      json['nonuser'].forEach((v) {
        nonuser.add(new Nonuser.fromJson(v));
      });
    }
  }
  ...
  Map<String, dynamic> toJson() {
    ...
    data['ok'] = this.ok;
    if (this.nonuser != null) {
      data['nonuser'] = this.nonuser.map((v) => v.toJson()).toList();
    }
    return data;
  }
}
...
class Nonuser {
  ...
  Nonuser.fromJson(Map<String, dynamic> json) {
    sentId = json['sent_id'];
